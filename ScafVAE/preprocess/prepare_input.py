...
from ScafVAE.preprocess.bind_preprocess import *
...
def prepare_frag(som_prepared_path, pred_ppl_path, idx):
    ...
    assert len(frag_mol_index_tup) <= global_config.max_frag_len
    ...
    frag_codebook = pd.read_csv(frag_codebook_path)['scaf_smi'].tolist()
...
def process_smi(smi_list, args, device='cpu', cache='./cache', batch_size=32):
    ...
    for dic_data in tqdm(test_loader, desc='Predicting bond ppl'):
        ...
        for i in range(len(dic_output['idx'])):
            ...
            np.savez_compressed(f'{cache}/ppl_pred/{idx}.npz', bond_ppl_ce=to_numpy(dic_output['bond_ppl_ce'][i]))
