...
class FragEncoder(torch.nn.Module):
    def __init__(self, model_config):
        ...
        if self.model_config.with_aux:
            fp_all_hidden = Aux_config.fp_mogan_hidden + Aux_config.fp_MACCS_hidden
    ...
    def train_encoder_aa(self, x_aa_init, edge_aa_init, dic_data):
        ...
        # x_pooling and edge_pooling can larger than max_frag_size due to padding
    ...
    def pooling_to_noise(self, dic_data, use_fp_dropout=False):
        if self.model_config.with_aux:
            fp = dic_data['fp']
            if use_fp_dropout:
                fp = F.dropout(fp, p=Aux_config.fp_dropout, training=True)  # training always True
...
class FragDecoder(torch.nn.Module):
    def __init__(self, model_config):
        super().__init__()
        self.model_config = model_config
        self.data_config = training_config.data
        ...
        self.df_scaf = pd.read_csv(frag_codebook_path)
        ...
        self.register_buffer(
            'dic_att_num',
            F.pad(
                torch.from_numpy(np.load(scaf_att_num_path)['all_att_num']),
                (0, 0, 1, 3), 'constant', 5,
            )
        )
    ...
    def train_decoder(self, dic_data):
        ...
        edge = torch.zeros(B, N, N, self.model_config.edge_hidden, device=device)
        for i in range(N):
            if i == 0:
...
class ModelBase(torch.nn.Module):
    def __init__(self, args, model_config=None):
        ...
        self.frag_encoder = FragEncoder(model_config)
        self.frag_decoder = FragDecoder(model_config)

        if self.model_config.with_aux:
            self.aux_proj_cl = torch.nn.Sequential(
                torch.nn.LayerNorm(model_config.noise_hidden),
                torch.nn.Linear(model_config.noise_hidden, Aux_config.cl_hidden),
                torch.nn.LeakyReLU(),
                torch.nn.LayerNorm(Aux_config.cl_hidden),
                torch.nn.Linear(Aux_config.cl_hidden, Aux_config.cl_hidden),
            )
            fp_all_hidden = Aux_config.fp_mogan_hidden + Aux_config.fp_MACCS_hidden
