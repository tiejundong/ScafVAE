...
def set_gpu_device(rank, world_size, port, args):
    if args.use_multi_gpu:
        ...
        # nccl gloo ,nccl no support to all_gather ?
        dist.init_process_group('nccl', rank=rank, world_size=world_size)  # nccl / gloo
...
def get_dataloader(args, train_dataset, val_dataset, world_size, collate_fn):
    if args.use_multi_gpu:
        train_sampler = torch.utils.data.distributed.DistributedSampler(
            train_dataset, num_replicas=world_size, rank=args.rank, shuffle=True)
        ...
        val_sampler = torch.utils.data.distributed.DistributedSampler(
            val_dataset, num_replicas=world_size, rank=args.rank, shuffle=False)
...
class CustomOptimization:
    def __init__(self, args, dic_opt):
        ...
        self.args = args
        self.params = dic_opt['params']

        self.use_amp_mix = args.use_amp_mix
        if args.use_amp_mix:  # might error in some design
        ...
        # clip only perform when amp_mix = False
        self.use_grad_clip = True if args.grad_clip_max_norm != -1 or args.grad_clip_value != 0 else False
        if args.grad_clip_max_norm != -1:
            self.grad_clip_type = 'norm'
            self.grad_clip_max_norm = args.grad_clip_max_norm
        elif args.grad_clip_value != 0:
            self.grad_clip_type = 'value'
            self.grad_clip_value = args.grad_clip_value

    def __call__(self, grad_loss, model=None, update_together=True, epoch=-1):
        '''
        opt model params
        :param grad_loss: grad_loss can be a tuple/list of losses, for single loss -> (loss_1, ) / [loss_1]
        :param update_together: If True, update param after calc all grad. If False, update grad for each optimizer
        :return: None
        '''
    ...
    def grad_clip(self, opt_step_i):
        if self.use_grad_clip:
            for g_index, group in enumerate(self.optimizers[opt_step_i].param_groups):
                params = group['params']
                ...
                if self.grad_clip_type == 'norm':
                    ...
                    torch.nn.utils.clip_grad_value_(params, clip_value=self.grad_clip_value)
...
def save_params(args, my_model, loss_object, opt_object, dic_traj, save_path, exclude_param=None):
...
def init_params(args, my_model, loss_object, opt_object, strict_param=True):
    if args.restart == 0:
        if args.choose_start_weight is not None:
            ...
            if args.use_multi_gpu:
                my_model.module.load_state_dict(chk['model_state_dict'], strict=strict_param)
            else:
            ...
            if 'loss_state_dict' in chk.keys():
                if args.use_multi_gpu_for_loss_object:
                    loss_object.module.load_state_dict(chk['loss_state_dict'], strict=strict_param)
                else:
                    loss_object.load_state_dict(chk['loss_state_dict'], strict=strict_param)
            del chk
        else:
        ...
        if args.rank == 0 or not args.use_multi_gpu:
            print('Restart at [{}/{}], current lr: {:.2e}'.format(args.restart,
                                                                  args.n_epoch,
                                                                  opt_object.optimizers[0].param_groups[0]['lr']))
        del chk
